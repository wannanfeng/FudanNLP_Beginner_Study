...
def NN_embedding(model, train, test, learning_rate, iter_times):
    optimizer = optim.Adam(model.parameters(), lr=learning_rate) # 定义优化器（求参数）
    loss_fun = F.cross_entropy  # 损失函数

    train_loss_record = list() # 损失值记录
    test_loss_record = list()

    train_record = list() # 准确率记录
    test_record = list()
    # torch.autograd.set_detect_anomaly(True)

    for iteration in range(iter_times):
        torch.cuda.empty_cache() # 用于清空 CUDA 缓存
        model.train()  # 进入训练模式
        for i, batch in enumerate(train):
            ...
            optimizer.step()
        with torch.no_grad():
            model.eval() # 进入非训练模式（测试模式）
            train_acc = list() # 正确率记录
            test_acc = list()
            train_loss = 0 # 损失值记录
            test_loss = 0
            for i, batch in enumerate(train):
                ...
                _, y_pre = torch.max(pred, -1)  #指定维度上计算最大值,返回(values, indices) (张量, 索引)
...
def NN_plot(random_embedding, glove_embedding, len_feature, len_hidden, learning_rate, batch_size, iter_times):
    ...
    #在初始化不同的模型和执行不同的训练任务之前 都需要再次调用这些随机种子
    random.seed(2021)
