...
class Named_Entity_Recognition(nn.Module):
    def __init__(self, len_feature, len_words, len_hidden, type_num, pad_id,
                 start_id, end_id, weight = None, drop_out = 0.5):
        super.__init__()
...
class CRF(nn.Module):
    def __init__(self, type_num, pad_id, start_id, end_id):
        ...
        self.transition = nn.Parameter(transition).cuda() #将一个 Tensor 转换成一个可训练的参数
